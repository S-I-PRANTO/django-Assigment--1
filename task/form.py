...
class Maxin:
    ...
    basestyle=' rounded-lg p-2 mb-2 '
    def apply_style(self):
        for field_name, field in self.fields.items():
            if isinstance(field.widget,forms.TextInput):
                field.widget.attrs.update({
                    'class':f"{self.basestyle} w-2/6 border ",
                    'placeholder':f"Enter {field.label.lower()}"
                })
            elif isinstance(field.widget,forms.Textarea):
                field.widget.attrs.update({
                    'class':f"{self.basestyle} w-4/6 border ",
                     'placeholder':f"Enter {field.label.lower()}"
                })
            else:
                 field.widget.attrs.update({
                     'class':f"{self.basestyle} border ",
...
class EventForm(Maxin,forms.ModelForm):
    class Meta:
        model=Event
        fields=['Event_Name','description','data','time','location','img_add']
        widgets = {
